...
#if defined(NDEBUG)
# error "Boingcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x35455d71ff2df72a5cc952b23febc3f3fc3aa67ac9ac43cc12bc2e9e035070bd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Boingcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Boingcoin Signed Message:\n";
...
{
    // Boingcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Boingcoin: 1000000 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 24 * 60 * 60; // Boingcoin: 5 days
static const int64 nTargetSpacing = 20 * 60; // Boingcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Boingcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Boingcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe72e87e7fec7277028e66e6e02320c4eb3e919eb4d3f6b09e55a450c760ae94a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "US Senate Hearing Rekindles Debate";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0410cebe96726dc18e0295525df446b570beb84b2e08694e767202fe0f707a6b9ce9a1d0c638192ddb1093519d5270ec4f976c4824e4a0b31a3ddb2662d8003c75") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517963823;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 389019090;

        if (fTestNet)
        {
            block.nTime    = 1517963805;
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf419a5e13e833f2f82595d06c516c07b7e8e86abd75dd3997ca599078635bc27"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xce, 0xbb, 0xd3 }; // Boingcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BoingcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BoingcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BoingcoinMiner(CWallet *pwallet)
{
    printf("BoingcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("boingcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BoingcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BoingcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BoingcoinMiner, pwallet));
}
